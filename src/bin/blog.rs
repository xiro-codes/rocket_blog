...
use app::{
    controllers, create_base_rocket_with_database,
...
use rocket::{catch, catchers, fs::FileServer, launch, response::Redirect, Build, Rocket};
...
impl BlogControllerRegistry {
    pub fn attach_all_controllers(rocket: Rocket<Build>) -> Rocket<Build> {
        ...
        log::debug!("Attaching controllers: Index (/), Auth (/auth), Blog (/blog), Comment (/comment), Feed (/feed), SEO (/)");
